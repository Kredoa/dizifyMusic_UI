...
const Artist = () => {
    ...
    function get4Albums(artistAlbums) {
        const shuffled = artistAlbums.sort(function(){return .5 - Math.random()});
        const sortedAlbums = shuffled.slice(0,4);
        return sortedAlbums.map((item, index) => (
            <AlbumCard id={item.id} item={item} />
        ))
    }
